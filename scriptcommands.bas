...
FUNCTION standard_embed_codes(act as string, byval arg as integer) as string
 ...
 SELECT CASE UCASE(act)
  ...
  CASE "S": '--string variable by ID
   insert = ""
   'Allow string constants (ID > 100), although they're nonsensical in Custom,
   'because the string we're expanding might have been generated by a script.
...
'old scriptmisc
 ...
 CASE 251'--set string from table
  'HSpeak is meant to catch out-of-range IDs
  IF bound_arg(retvals(0), 0, maxScriptString, "string ID", !"$# = \"...\"", serrBadOp) THEN
...
'old scriptnpc
 ...
 CASE 681 '--joystick hat count (joynum)
  IF valid_joystick(retvals(0)) THEN
   ...
   END IF
  END IF
 CASE 682 '--intern string from table
  'HSpeak is meant to catch out-of-range IDs
  IF bound_arg(retvals(0), 0, maxScriptString, "string ID", "$""...""", serrBadOp) THEN
   plotstr(retvals(0)).s = script_string_constant(nowscript, retvals(0))
   scriptret = retvals(0)
  END IF
