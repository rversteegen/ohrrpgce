...
FUNCTION create_cure_attack(stat_num as integer, amount as integer, allow_exceed_max as bool = NO) as integer
  ...
  initattackdata atk
  atk.name = "+" & amount & " " & LEFT(statnames(stat_num), 5)
  atk.description = "Restore " & amount & " " & statnames(stat_num)
  atk.damage_math = 3                    'Pure Damage
  atk.targ_stat = stat_num
  atk.extra_damage = amount - 100        '% modifier
  atk.randomization = 0
  atk.cure_instead_of_harm = YES
  atk.ignore_extra_hits = YES
  atk.do_not_randomize = YES             'Obsolete but set for compatibility
  atk.damage_can_be_zero = YES
  atk.ignore_damage_cap = YES
  atk.allow_cure_to_exceed_maximum = allow_exceed_max
  gen(genMaxAttack) += 1
  DIM new_atk_id as integer = gen(genMaxAttack)
  saveattackdata atk, new_atk_id
  RETURN new_atk_id
END FUNCTION
